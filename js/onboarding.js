import { CAMPO_CODES, ACCESS_KEY } from './config.js';
...
export function setupStep2() {
  ...
  document.getElementById('ob-step2-equipa').style.display = ['jogador','treinador','dirigente'].includes(f) ? 'block' : 'none';
...
function setupStep3() {
  const needsKey = ['treinador','dirigente'].includes(state.profile.funcao);
...
export function handleStep3() {
  const f = state.profile.funcao;
  if (['treinador','dirigente'].includes(f)) {
